...
*********************/
add_action( 'after_setup_theme', 'test_build', 16 );

function test_build() {

	add_action('init', 'test_head_cleanup');										// launching operation cleanup
	add_filter('the_generator', 'test_rss_version');								// remove WP version from RSS
	add_filter( 'wp_head', 'test_remove_wp_widget_recent_comments_style', 1 );	// remove pesky injected css for recent comments widget
	add_action('wp_head', 'test_remove_recent_comments_style', 1);				// clean up comment styles in the head
	add_filter('gallery_style', 'test_gallery_style');							// clean up gallery output in wp
	add_action('wp_enqueue_scripts', 'test_scripts_and_styles', 999);			// enqueue base scripts and styles

	test_theme_support();														// launching this stuff after theme setup
	add_action( 'widgets_init', 'test_register_sidebars' );						// adding sidebars to Wordpress (these are created in functions.php)
	add_filter( 'get_search_form', 'test_wpsearch' ); 							// adding the scaffolding search form (created in functions.php)
	add_filter('the_content', 'test_filter_ptags_on_images'); 					// cleaning up random code around images
	add_filter('excerpt_more', 'test_excerpt_more');								// cleaning up excerpt
}
...
function test_head_cleanup() {
	...
	add_filter( 'style_loader_src', 'test_remove_wp_ver_css_js', 9999 );	// remove WP version from css
	add_filter( 'script_loader_src', 'test_remove_wp_ver_css_js', 9999 );// remove WP version from scripts
}

// remove WP version from RSS
function test_rss_version() { return ''; }

// remove WP version from scripts
function test_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) )
...
function test_remove_wp_widget_recent_comments_style() {
...
function test_gallery_style($css) {
...
function test_page_navi($before = '', $after = '') {
	...
	echo $before.'<nav class="page-navigation"><ol class="test_page_navi clearfix">'."";
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( "First", 'test' );
...
function test_get_next_posts_link_attributes($attr){
	$attr = 'rel="next" title="View the Next Page"';
	return $attr;
}
add_filter('next_posts_link_attributes', 'test_get_next_posts_link_attributes');

//add rel and title attribute to prev pagination link
function test_get_previous_posts_link_attributes($attr){
	$attr = 'rel="prev" title="View the Previous Page"';
	return $attr;
}
add_filter('previous_posts_link_attributes', 'test_get_previous_posts_link_attributes');

/*********************
CLIENT UX FUNCTIONS
*********************/
//Extend permisitons for the 'editor' role (used for client accounts)
function test_increase_editor_permissions(){
...
add_action('admin_init','test_increase_editor_permissions');
...
function test_remove_admin_bar_links() {
	global $wp_admin_bar;
...
add_action( 'wp_before_admin_bar_render', 'test_remove_admin_bar_links' );

// Custom Backend Footer
function test_custom_admin_footer() {
...
add_filter('admin_footer_text', 'test_custom_admin_footer');

// CUSTOM LOGIN PAGE
// calling your own login css so you can style it
function test_login_css() {
...
function test_login_url() {  return home_url(); }

// changing the alt text on the logo to show your site name
function test_login_title() { return get_option('blogname'); }

// calling it only on the login page
add_action('login_head', 'test_login_css');
add_filter('login_headerurl', 'test_login_url');
add_filter('login_headertitle', 'test_login_title');

//Add page title attribute to a tags
function test_wp_list_pages_filter($output) {
	$output = preg_replace('/<a(.*)href="([^"]*)"(.*)>(.*)<\/a>/','<a$1 title="$4" href="$2"$3>$4</a>',$output);
	return $output;
}
add_filter('wp_list_pages', 'test_wp_list_pages_filter');

//return the search results page even if the query is empty - http://vinayp.com.np/how-to-show-blank-search-on-wordpress/
function test_make_blank_search ($query){
	global $wp_query;
...
add_action('pre_get_posts','test_make_blank_search');

/*********************
DASHBOARD WIDGETS
*********************/
// disable default dashboard widgets
function test_disable_default_dashboard_widgets() {
...
add_action('admin_menu', 'test_disable_default_dashboard_widgets');

/*********************
VISITOR/USER UX FUNCTIONS
*********************/
//Apply styles to the visual editor
function test_mcekit_editor_style($url) {
	if ( !empty($url) ) {
		$url .= ',';
	}
...
add_filter('mce_css', 'test_mcekit_editor_style');
...
function test_remove_img_dimensions($html) {
...
add_filter('post_thumbnail_html', 'test_remove_img_dimensions', 10);
//add_filter('the_content', 'test_remove_img_dimensions', 10); //Options - This has been removed from the content filter so that clients can still edit image sizes in the editor
add_filter('get_avatar','test_remove_img_dimensions', 10);

// remove the p from around imgs (http://css-tricks.com/snippets/wordpress/remove-paragraph-tags-from-around-images/)
function test_filter_ptags_on_images($content){
	return preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
}

// Fix Gravity Form Tabindex Conflicts - http://gravitywiz.com/2013/01/28/fix-gravity-form-tabindex-conflicts/
function test_gform_tabindexer() {
	$starting_index = 1000; // if you need a higher tabindex, update this number
	return GFCommon::$tab_index >= $starting_index ? GFCommon::$tab_index : $starting_index;
}
add_filter("gform_tabindex", "test_gform_tabindexer");

// Filter out hard-coded width, height attributes on all captions (wp-caption class)
add_shortcode('wp_caption', 'test_fixed_img_caption_shortcode');
add_shortcode('caption', 'test_fixed_img_caption_shortcode');
